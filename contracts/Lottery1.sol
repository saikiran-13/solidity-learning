...
contract Lottery{
    ...
    function getTicket(uint _amount) external{
             ...
             require(token.transferFrom(msg.sender,address(this),_amount),"Transaction unsuccessful");
    ...
    function random() private view  returns (uint){
        return uint(keccak256(abi.encodePacked(block.number,block.prevrandao,block.timestamp,participants.length)));
    }
    ...
    function findWinner() external onlyOwner{
        ...
        require(token.transfer(payable(winner),showBalance()),"Transaction unsuccessful");
